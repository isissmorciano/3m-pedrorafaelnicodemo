CODICI_COMUNI = ["abc12345", "student01", "prova000", "admin999"]


def analizza_numeri(n: int, numeri: list[int]) -> tuple[list[int], int, float]:
    if n <= 0:
        raise ValueError("n deve essere maggiore di 0")
    if len(numeri) != n:
        raise ValueError("numero di valori diverso da n")

    positivi_pari = []
    count_dispari_negativi = 0
    somma_assoluti = 0

    for num in numeri:
        if num > 0 and num % 2 == 0:
            positivi_pari.append(num)
        if num < 0 and num % 2 != 0:
            count_dispari_negativi += 1
        somma_assoluti += abs(num)

    media_assoluta = somma_assoluti / n
    return positivi_pari, count_dispari_negativi, media_assoluta


def analizza_codice(codice: str) -> dict[str, int]:
    maiuscole = 0
    minuscole = 0
    cifre = 0

    for carattere in codice:
        if carattere.isupper():
            maiuscole += 1
        elif carattere.islower():
            minuscole += 1
        elif carattere.isdigit():
            cifre += 1

    return {"maiuscole": maiuscole, "minuscole": minuscole, "cifre": cifre}


def valida_codice(codice: str) -> bool:
    if len(codice) < 8:
        return False

    if not codice:
        return False

    if not codice[0].isupper():
        return False

    if not codice.isalnum():
        return False

    if codice.lower() in CODICI_COMUNI:
        return False

    conteggi = analizza_codice(codice)
    if conteggi["minuscole"] < 1 or conteggi["cifre"] < 1:
        return False

    return True


def livello_codice(codice: str) -> int:
    if not valida_codice(codice):
        return 0

    livello = 1
    if len(codice) >= 10:
        livello += 1

    conteggi = analizza_codice(codice)
    if conteggi["maiuscole"] > 1 or conteggi["cifre"] > 1:
        livello += 1

    return min(livello, 3)


def esercizio1_main() -> None:
    n = int(input("Quanti numeri inserire? "))
    numeri: list[int] = []
    for i in range(n):
        numeri.append(int(input(f"Numero {i + 1}: ")))

    positivi_pari, count_dispari_negativi, media_assoluta = analizza_numeri(n, numeri)
    print(f"Lista inserita: {numeri}")
    print(f"Positivi pari: {positivi_pari}")
    print(f"Conteggio dispari negativi: {count_dispari_negativi}")
    print(f"Media assoluta: {media_assoluta}")


def esercizio2_main() -> None:
    codice = input("Inserisci un codice alfanumerico: ").strip()
    if valida_codice(codice):
        print(f"Valido (Livello {livello_codice(codice)})")
    else:
        print("Non valido")


def main() -> None:
    print("--- Verifica di Recupero Primo Quadrimestre ---")
    print("\n-- Esercizio 1: Cicli, liste e statistiche --")
    try:
        esercizio1_main()
    except ValueError as exc:
        print(f"Errore: {exc}")

    print("\n-- Esercizio 2: Funzioni e validazione di un codice alfanumerico --")
    esercizio2_main()


if __name__ == "__main__":
    main()
